...
-define(TYPE_BYTECODE, 2#10010111). %% 1010 0111 - Bytecode typedef
                                    %% 1011 0111
                                    %% 1100 0111
                                    %% 1101 0111
...
-define(BYTECODE     , 2#10001111). %% 1000 1111
...
serialize(?FATE_BYTECODE(B)) ->
    <<?BYTECODE,
      (serialize_integer(?FATE_BYTECODE_SIZE(B)))/binary
     , B/binary>>.
...
serialize_type(bytecode) -> [?TYPE_BYTECODE].
...
deserialize_type(<<?TYPE_BYTECODE, Rest/binary>>) -> {bytecode, Rest}.
...
deserialize2(<<?BYTECODE, Rest/binary>>) ->
    {Size, Rest2} = deserialize_one(Rest),
    true = is_integer(Size) andalso Size >= 0,
    ByteCode = binary:part(Rest2, 0, Size),
    Rest3 = binary:part(Rest2, byte_size(Rest2), - (byte_size(Rest2) - Size)),
    {?MAKE_FATE_BYTECODE(ByteCode), Rest3};
