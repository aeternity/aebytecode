...
ops_defs() ->
    ...
    , { 'CREATE_G',            16#a4,   true,   false,   true, 10000,   [a,a,a,a],      create_g, {contract_bytearray, typerep, integer, integer}, contract, "Like `CREATE` but additionally limits gas of `init` call to Arg3"}
    , { 'CLONE',               16#a5,   true,   false,   true,  1000,   [a,a,a,a],         clone,              {contract, typerep, integer, bool},      any, "Clones the contract under Arg1 and deploys it with value of Arg3. The `init` arguments should be placed on the stack and match the type in Arg2. Writes contract (or `None` on fail when protected) to stack top."}
    , { 'CLONE_G',             16#a6,   true,   false,   true,  1000, [a,a,a,a,a],       clone_g,     {contract, typerep, integer, integer, bool},      any, "Like `CLONE` but additionally limits gas of `init` call to Arg3"}
    , { 'BYTECODE_HASH',       16#a7,  false,    true,   true,   100,       [a,a], bytecode_hash,                                      {contract},  variant, "Arg0 := hash of the deserialized contract's bytecode under address given in Arg1 (or `None` on fail)."}
